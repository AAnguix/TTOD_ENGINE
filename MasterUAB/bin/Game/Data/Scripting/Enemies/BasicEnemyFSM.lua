...
function OnUpdate_Idle_BasicEnemy(Enemy, ElapsedTime)
	...
	if CTTODMathUtils.PointInsideCircle(l_PlayerPos, l_Pos, Enemy.m_VisionRange) and Enemy:GetTimer() > Enemy:GetAttackDelay()  then 
		if CTTODMathUtils.PointInsideCircle(l_PlayerPos, Enemy.m_RObject:GetPosition(), Enemy.m_AttackRange) then
...
function OnUpdate_Patrol_BasicEnemy(Enemy, ElapsedTime)
	...
	Enemy:SetFTWP(true)
	if CTTODMathUtils.PointInsideCircle(g_Player:GetRenderableObject():GetPosition(), Enemy.m_RObject:GetPosition(), Enemy.m_VisionRange) then
		Enemy:GetAnimator():SetBool("IsPlayerInsideVisionRange", true)
	end
end
