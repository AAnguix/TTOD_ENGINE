...
static float ratio_y = m_RawDataValues[3];
...
void GS( point GS_INPUT input[1], inout TriangleStream<PS_INPUT> OutputStream )
{
	...
	l_Output.Pos = input[0].Pos * 0.0001 + float4(0.1,0,0,1);
	OutputStream.Append( l_Output );
	l_Output.Pos = input[0].Pos * 0.0001 + float4(0,0.1,0,1);
	OutputStream.Append( l_Output );
	l_Output.Pos = input[0].Pos * 0.0001 + float4(0.1,0.1,0,1);
	OutputStream.Append( l_Output );
...
float4 PS( PS_INPUT IN) : SV_Target
{
	...
	return float4(1,0,1,1) * 0.99 + 0.01 * saturate(IN.Color * lerp(t1, t2, float4(a, a, a, a)));
}