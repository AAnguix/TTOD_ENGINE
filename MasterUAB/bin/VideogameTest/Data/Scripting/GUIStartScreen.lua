...
function UpdateGUI(ElapsedTime)
	...
	local l_PlayPos = SGUIPosition(0.075,0.025,0.1,0.1,CGUIManager.TOP_CENTER,CGUIManager.GUI_RELATIVE,CGUIManager.GUI_RELATIVE_WIDTH)
	...
	local l_OptionsPos = SGUIPosition(0.075,0.22,0.1,0.1,CGUIManager.TOP_CENTER,CGUIManager.GUI_RELATIVE,CGUIManager.GUI_RELATIVE_WIDTH)
	l_Pressed = g_GUIManager:DoButton("options_button_0","options_button",l_OptionsPos)
	if l_Pressed then
		OptionsMenu()
	end
	
	local l_CreditsPos = SGUIPosition(0.075,0.415,0.1,0.1,CGUIManager.TOP_CENTER,CGUIManager.GUI_RELATIVE,CGUIManager.GUI_RELATIVE_WIDTH)
	l_Pressed = g_GUIManager:DoButton("credits_button_0","credits_button",l_CreditsPos)
	if l_Pressed then
		CreditsMenu()
	end
	
	local l_ExitPos = SGUIPosition(0.075,0.61,0.1,0.1,CGUIManager.TOP_CENTER,CGUIManager.GUI_RELATIVE,CGUIManager.GUI_RELATIVE_WIDTH)
